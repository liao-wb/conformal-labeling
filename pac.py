...
for j in range(num_trials):
    fdp, power, selection_size = selection(Y, Yhat, confidence, alpha, calib_ratio=args.calib_ratio, random=(args.random == "True"), args=args)
    fdr_list.append(fdp)
...
print(f"Results of {dataset} dataset. q = {args.alpha}")
...
print(f"Error: {np.mean(error_list) * 100}")
print(f"Accuracy:{np.sum(Y==Yhat) / len(Y)}")
