...
parser.add_argument("--datasets", type=str, default="text",
                    choices=["vision", "text", "all", 'stance', 'misinfo', 'bias', 'imagenet', 'imagenetv2'])
...
selection_size_array = np.zeros(shape=(len(ds_list), len(alpha_list)))
...
for i, ds in enumerate(ds_list):
    ...
    for j, alpha in enumerate(alpha_list):
        ...
        for z in range(num_trials):
            fdp, power, selection_size = selection(Y, Yhat, confidence, alpha, calib_ratio=0.5,
